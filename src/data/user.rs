...
impl DBUser {
	pub fn update_required_xp(&mut self) { self.xp_until_next_level = 100 + self.level.pow(2) / 2 }
	...
	pub fn give_item(&mut self, item: InventoryItem) { self.items.push(item); }
